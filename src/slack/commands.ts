import type { BacklogItem, Priority, SlackCommand, SlackState, TaskStatus } from '../types.js';
import { BacklogStore, generateTaskId } from '../backlog/store.js';

const PRIORITY_VALUES: Priority[] = ['high', 'medium', 'low'];
const STATUS_VALUES: TaskStatus[] = ['pending', 'in-progress', 'done', 'blocked'];
...
function formatTaskDetail(item: BacklogItem): string {
  ...
  if (item.body) {
    const truncated = item.body.length > 1500 ? item.body.slice(0, 1500) + '...' : item.body;
...
export function parseCommand(text: string): SlackCommand | null {
  ...
  if (/^list\b/i.test(trimmed)) {
    const status = extractFlag(trimmed, 'status')?.toLowerCase() as TaskStatus | undefined;
    const priority = extractFlag(trimmed, 'priority')?.toLowerCase() as Priority | undefined;
  ...
  if (addMatch) {
    ...
    const priority = (extractFlag(raw, 'priority')?.toLowerCase() as Priority) ?? 'medium';
...
export function executeCommands(
  ...
  for (const cmd of commands) {
    switch (cmd.type) {
      ...
      case 'list': {
        let visible = cmd.status ? items : items.filter((i) => i.status !== 'done');
        if (cmd.status) visible = visible.filter((i) => i.status === cmd.status);
        if (cmd.priority) visible = visible.filter((i) => i.priority === cmd.priority);
        ...
        responses.push(`:clipboard: *Backlog (${visible.length})*\n${lines.join('\n')}`);
        break;
      }
      ...
      case 'add': {
        ...
        try {
          ...
          responses.push(`:memo: Created *${cmd.title}* (\`${id}\`) as ${cmd.priority} priority.`);
        } catch (err) {
