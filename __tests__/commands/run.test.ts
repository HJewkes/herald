...
function resetSlackClientMocks() {
  ...
  mockSlackClient.uploadFile.mockResolvedValue({
    id: "F1",
    name: "f.txt",
    permalink: "",
  });
...
describe("run command", () => {
  beforeEach(() => {
    ...
    vi.mocked(BacklogStore).mockImplementation(
      () => mockStore as unknown as BacklogStore,
    );
    vi.mocked(checkBudget).mockReturnValue(healthyBudget);
    vi.mocked(SlackClient).mockImplementation(
      () => mockSlackClient as unknown as InstanceType<typeof SlackClient>,
    );
  ...
  it("happy path: selects task, executes, journals, and notifies", async () => {
    ...
    expect(mockStore.updateLastRun).toHaveBeenCalledWith(
      task.filePath,
      expect.any(String),
    );
  ...
  it("exits early when lock acquisition fails", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      expect.stringContaining("Another herald run"),
    );
    logSpy.mockRestore();
  });
  ...
  it("recovers orphaned in-progress tasks to pending", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      expect.stringContaining("Recovering orphaned task"),
    );
    logSpy.mockRestore();
  });
  ...
  it("sets recurring task back to pending on completion, not done", async () => {
    ...
    const finalStatusCall = statusCalls.find(
      ([path, status]: [string, string]) =>
        path === recurring.filePath && status !== "in-progress",
    );
...
describe("processReactions", () => {
  ...
  it("sets pause on pause_button reaction", async () => {
    ...
    const client = {
      getReactions: vi
        .fn()
        .mockResolvedValue([{ name: "pause_button", users: ["U1"] }]),
    };
    const state: SlackState = {
      lastCheckedTs: "0",
      pauseRequested: false,
    ...
    const store = {
      list: vi.fn().mockReturnValue({ items: [], warnings: [] }),
    };
