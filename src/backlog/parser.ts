...
export function parseBacklogItem(
  content: string,
  filePath: string,
): BacklogItem {
  ...
  if (!VALID_TASK_TYPES.has(typeVal)) {
    throw new Error(
      `${filePath}: invalid type: "${typeVal}" (expected: ${[...VALID_TASK_TYPES].join(", ")})`,
    );
  }
  ...
  if (!VALID_PRIORITIES.has(priorityVal)) {
    throw new Error(
      `${filePath}: invalid priority: "${priorityVal}" (expected: ${[...VALID_PRIORITIES].join(", ")})`,
    );
  }
  ...
  if (!VALID_STATUSES.has(statusVal)) {
    throw new Error(
      `${filePath}: invalid status: "${statusVal}" (expected: ${[...VALID_STATUSES].join(", ")})`,
    );
  }
