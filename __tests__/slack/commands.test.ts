...
describe("parseCommand", () => {
  ...
  it('parses "skip <taskId>"', () => {
    expect(parseCommand("skip task-001")).toEqual({
      type: "skip",
      taskId: "task-001",
    });
  });
  ...
  it('parses "unblock <taskId>"', () => {
    expect(parseCommand("unblock task-001")).toEqual({
      type: "unblock",
      taskId: "task-001",
    });
  });
  ...
  it("keywords are case insensitive but task IDs are preserved", () => {
    expect(parseCommand("PAUSE")).toEqual({ type: "pause" });
    expect(parseCommand("Skip TASK-ABC-001")).toEqual({
      type: "skip",
      taskId: "TASK-ABC-001",
    });
  ...
  it('parses "list --status blocked"', () => {
    expect(parseCommand("list --status blocked")).toEqual({
      type: "list",
      status: "blocked",
      priority: undefined,
      tag: undefined,
  ...
  it('parses "list --priority high"', () => {
    expect(parseCommand("list --priority high")).toEqual({
      type: "list",
      status: undefined,
      priority: "high",
      tag: undefined,
  ...
  it('parses "list --tag refactor"', () => {
    expect(parseCommand("list --tag refactor")).toEqual({
      type: "list",
      status: undefined,
      priority: undefined,
      tag: "refactor",
  ...
  it('parses "show <taskId>"', () => {
    expect(parseCommand("show task-001")).toEqual({
      type: "show",
      taskId: "task-001",
    });
  });
