...
describe("loadConfig", () => {
  ...
  it("throws clear error on invalid JSON", () => {
    ...
    expect(() => loadConfig("/fake/project")).toThrow(
      "Invalid JSON in herald.config.json",
    );
  });
  ...
  it("throws when merged config has invalid shape", () => {
    ...
    expect(() => loadConfig("/fake/project")).toThrow(
      "Invalid config: budget must be an object",
    );
  });
  ...
  it("throws when notify.slack is missing", () => {
    ...
    expect(() => loadConfig("/fake/project")).toThrow(
      "Invalid config: notify.slack must be an object",
    );
  });
  ...
  it("throws when budget fields have wrong types", () => {
    ...
    expect(() => loadConfig("/fake/project")).toThrow(
      "Invalid config: budget.weeklyTokenLimit must be a number",
    );
  });
});
