...
describe('SlackClient', () => {
  ...
  it('postMessage returns ts and channel', async () => {
    vi.stubGlobal('fetch', mockFetch({ ok: true, ts: '1234.5678', channel: 'C123' }));
  ...
  it('postMessage throws when ts is missing from response', async () => {
    ...
    await expect(client.postMessage('#herald', 'hello')).rejects.toThrow('missing ts');
  });
  ...
  it('updateMessage calls chat.update', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        body: JSON.stringify({ channel: 'C123', ts: '1234.5678', text: 'updated text' }),
      }),
    );
  });
  ...
  it('addReaction calls reactions.add', async () => {
    ...
    const body = JSON.parse(fetcher.mock.calls[0][1].body);
    expect(body).toEqual({ channel: 'C123', timestamp: '1234.5678', name: 'thumbsup' });
  });
  ...
  it('getHistory returns parsed messages', async () => {
    ...
    expect(messages[0]).toEqual({ ts: '1.0', user: 'U1', text: 'hello', threadTs: undefined });
  ...
  it('uploadFile uses getUploadURLExternal and completeUploadExternal', async () => {
    const fetcher = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ ok: true, upload_url: 'https://files.slack.com/upload/v1/abc', file_id: 'F1' }),
      })
    ...
    const result = await client.uploadFile('C123', 'file content', 'output.txt', '1234.5678');

    expect(result.id).toBe('F1');
    expect(result.name).toBe('output.txt');
    expect(fetcher.mock.calls[0][0]).toContain('files.getUploadURLExternal');
    expect(fetcher.mock.calls[1][0]).toBe('https://files.slack.com/upload/v1/abc');
  ...
  it('createChannel throws when response has no channel object', async () => {
    ...
    await expect(client.createChannel('herald-brain')).rejects.toThrow('missing channel.id');
  });
  ...
  it('throws on API error with method name', async () => {
    vi.stubGlobal('fetch', mockFetch({ ok: false, error: 'channel_not_found' }));
    const client = new SlackClient('test-token');
