...
describe('BacklogStore', () => {
  ...
  it('creates directory if it does not exist', () => {
    ...
    expect(vi.mocked(mkdirSync)).toHaveBeenCalledWith('/fake/backlog/active', { recursive: true });
  });
});
...
describe('generateTaskId', () => {
  ...
  it('returns 001 when no files exist for that date', () => {
    ...
    const id = generateTaskId('/fake/backlog', new Date('2026-02-22T12:00:00Z'));
    expect(id).toBe('2026-02-22-001');
  });

  it('returns 002 when one file for that date exists', () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readdirSync).mockReturnValue(['2026-02-22-001.md'] as unknown as ReturnType<typeof readdirSync>);
    const id = generateTaskId('/fake/backlog', new Date('2026-02-22T12:00:00Z'));
  ...
  it('ignores files from other dates', () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readdirSync).mockReturnValue(['2026-02-21-001.md', '2026-02-21-002.md'] as unknown as ReturnType<typeof readdirSync>);
    const id = generateTaskId('/fake/backlog', new Date('2026-02-22T12:00:00Z'));
    expect(id).toBe('2026-02-22-001');
  });
  ...
  it('handles gaps from deleted files by using max suffix', () => {
    ...
    vi.mocked(readdirSync).mockReturnValue(
      ['2026-02-22-001.md', '2026-02-22-003.md'] as unknown as ReturnType<typeof readdirSync>,
    );
    const id = generateTaskId('/fake/backlog', new Date('2026-02-22T12:00:00Z'));
